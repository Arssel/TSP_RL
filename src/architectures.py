...
class LinearEmbedding(nn.Module):
    ...
    def forward(self, x):
        ...
        return self.projection(x) + positional_encoding(batch_size, self.dm, n).to(x.device)
...
class DecoderActor(nn.Module):
    ...
    def forward(self, x):
        ...
        M_hat -= (torch.eye(n)*1e20).to(x.device)
        
        return F.softmax(M_hat.view(-1, n**2), dim=1).view(-1, n, n)

    
class Actor(nn.Module):
    def __init__(self, dm=128, dk=128, dq=128, dv=128, ff_dim=512, N=3, C=10):
        super().__init__()
        
        self.linear_embedding = LinearEmbedding(dm=dm)
        self.encoder = Encoder(dm=dm, dk=dk, dq=dq, dv=dv, ff_dim=ff_dim, N=N)
        self.decoder = DecoderActor(dm=dm, C=C)
